...
void ImageProcessing::open(string addr){
    cap.initialize(addr);
    cap.startThread();
}

void ImageProcessing::open(int addr){
    cap.initialize(addr);
    cap.startThread();
}
...
void ImageProcessing::saveImg(string name) {
    imwrite(name, preProcessed);
}

void ImageProcessing::rec(string name){
    go=1;
    writer.open(name,cv::VideoWriter::fourcc('M','J','P','G'),25, Size(960,720),true);
    for(;;){
        if(go){
...
void ImageProcessing::startRec(String name){
    record=thread(&ImageProcessing::rec,this,name);
    record.detach();
}
